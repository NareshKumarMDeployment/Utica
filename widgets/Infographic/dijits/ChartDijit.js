...
define([
...
], function(lang, html, array, declare, BaseDijit, moment, StatisticsChart, DataSourceManager, LayerInfos){
  ...
  var clazz = declare([BaseDijit], {
    ...
    _showNodata: function(type){
      html.addClass(this.domNode, 'no-data');
      if(type === 'max'){
        this.noDataDiv.innerHTML = 'Failed to compute. Too many time intervals due to the short parsing period.';
      }
      this.chart.clear();
    },
    ...
    _showMockData: function(chartConfig){
      ...
      this.chart.resizeByParent();
      if(this._checkIsMaxLabels([mockFeature], chartConfig)){
        return this._showNodata('max');
      }
      this.chart.createClientCharts(mockDefinition, [mockFeature], chartConfig, this.popupFieldInfosObj);
    },
    ...
    _tryUpdateStatisticsChartForClientFeatures: function(chartConfig){
      ...
      this.chart.resizeByParent();
      if(this._checkIsMaxLabels(features, chartConfig)){
        return this._showNodata('max');
      }
      ...
      this.chart.createClientCharts(this.featureLayer, features, chartConfig,
        this.popupFieldInfosObj, featureLayerForChartSymbologyChart);
    },
    ...
    _tryUpdateStatisticsChartForFrameworkFeatures: function(chartConfig){
      ...
      var featureLayerForChartSymbologyChart = null;
      if(chartConfig.useLayerSymbology !== 'undefined'){
    ...
    _tryUpdateStatisticsChartForFrameworkStatistics: function(chartConfig){
      ...
      this.chart.resizeByParent();
      if(this._checkIsMaxLabels(this.data.features, chartConfig)){
        return this._showNodata('max');
      }
      this.chart.createServerStatisticsCharts(dsMeta.dataSchema, this.data.features, chartConfig);
    },

    _checkIsMaxLabels: function(features, chartConfig){
